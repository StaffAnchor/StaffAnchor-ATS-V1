...
import PublicLocationSelector from '../components/PublicLocationSelector';
...
const PublicCandidateForm = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Build preferred locations from selected locations (only if they exist)
      const preferredLocations = selectedPreferredLocations.length > 0
        ? selectedPreferredLocations.map(locStr => {
      ...
      const response = await axios.post(`${API_URL}/api/candidates/public/submit`, candidateData);
      ...
      toast.error(error.response?.data?.error || 'Failed to submit your profile. Please try again.');
    } finally {
      setSubmitting(false);
    }
  };
            ...
            <Box sx={{ 
              ...
              <Box sx={{ width: '100%' }}>
                ...
                <Typography variant="body1" sx={{ 
                  ...
                  Submit your profile and we'll match you with the best opportunities
                </Typography>
              </Box>
            </Box>
          </CardContent>
            ...
            <form onSubmit={handleSubmit}>
              <Stack spacing={{ xs: 2, sm: 3 }}>
                ...
                <Box>
                  ...
                  <PublicLocationSelector
                    selectedLocations={selectedPreferredLocations}
                    onLocationsChange={setSelectedPreferredLocations}
                    multiple={true}
                    required={false}
                  />
                </Box>
                ...
                <Box sx={{ textAlign: 'center', pt: { xs: 2, sm: 3, md: 4 } }}>
                  <Paper sx={{ 
                    ...
                    <Typography variant="body2" sx={{ 
                      ...
                      By submitting this form, you agree to be contacted by StaffAnchor regarding career opportunities.
                    </Typography>
                    <Button 
                      type="submit"
                      variant="contained"
